...
type ProjectWithLinks = (typeof data.projects)[number] & { link?: string; relatedLinks?: RelatedLink[] };

export function generateStaticParams() { return data.projects.map(project => ({ slug: project.slug })); }
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const project = data.projects.find(item => item.slug === slug) as ProjectWithLinks | undefined; if (!project) notFound();
  ...
  const relatedLinks=[...(project.relatedLinks ?? []), ...(project.link ? [{ label: '関連リンク', url: project.link }] : [])];
  return <main className="inner-page project-detail"><a className="brand" href="/official/"><span>T</span> Taisei / PORTFOLIO</a><p className="section-kicker">PROJECT / {project.title}</p><h1>{project.title}</h1>{elapsedYears>=1&&<p className="article-age-notice">この記事は最終更新から{elapsedYears}年経過しています。内容が古くなっている可能性があります。</p>}<p className="lead">{project.description}</p><div className="project-tags">{project.tags.map(tag => <em key={tag}>{tag}</em>)}</div>{relatedLinks.length>0&&<section className="related-links"><p className="section-kicker">RELATED LINKS</p>{relatedLinks.map((item,index)=><a href={item.url} target="_blank" rel="noreferrer" key={`${item.url}-${index}`}>{item.label || item.url}<b>↗</b></a>)}</section>}<article className="tech-article"><p className="section-kicker">TECHNICAL NOTE</p><h2>{project.articleTitle}</h2><MermaidContent content={project.articleBody}/></article><a className="text-link" href="/official/works/">← プロジェクト一覧へ戻る</a></main>;
}
