...
export default function Home(){
 const [active,setActive]=useState(0),[ratio,setRatio]=useState('16 / 9'),[menuOpen,setMenuOpen]=useState(false);
 useEffect(()=>{const timer=setInterval(()=>setActive(i=>(i+1)%slides.slides.length),4500);return()=>clearInterval(timer)},[]);
 return <main>
  <header className="header"><a className="brand" href={base+'/' }><span>T</span> Taisei / PORTFOLIO</a><button className="menu" type="button" aria-expanded={menuOpen} aria-controls="site-navigation" onClick={()=>setMenuOpen(open=>!open)}>{menuOpen?'CLOSE':'MENU'}</button><nav id="site-navigation" className={menuOpen?'open':''}><a href={base+'/about/'} onClick={()=>setMenuOpen(false)}>ABOUT</a><a href={base+'/works/'} onClick={()=>setMenuOpen(false)}>WORKS</a><a href={base+'/resources/'} onClick={()=>setMenuOpen(false)}>RESOURCES</a><a href="https://ktaisei.github.io/blog/">BLOG ↗</a><a href={base+'/contact/'} onClick={()=>setMenuOpen(false)}>CONTACT</a></nav></header>
