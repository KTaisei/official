...
export default function Home(){
 ...
 return <main>
  <header className="header"><a className="brand" href={base+'/' }><span>T</span> Taisei / PORTFOLIO</a><nav><a href={base+'/about/'}>ABOUT</a><a href={base+'/works/'}>WORKS</a><a href="https://ktaisei.github.io/blog/">BLOG ↗</a><a href={base+'/contact/'}>CONTACT</a></nav></header>
  ...
  <footer><div className="footer-brand"><span>T</span><p>Taisei / PORTFOLIO</p></div><div className="footer-links"><div><b>NAVIGATE</b><a href={base+'/about/'}>About</a><a href={base+'/works/'}>Works</a><a href={base+'/contact/'}>Contact</a></div><div><b>FOLLOW</b>{site.socials.map(link=><a href={link.url} target="_blank" key={link.label}>{link.label}</a>)}</div></div><small>{site.footerCopyright}</small></footer>
 </main>
}
